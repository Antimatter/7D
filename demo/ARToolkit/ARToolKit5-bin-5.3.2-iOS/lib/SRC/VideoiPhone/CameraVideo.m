...
- (void) startAsync:(CameraVideoStartAsyncCompletionBlock)completion
{
    ...
    for (captureDevice in captureDevices) {
        if ([captureDevice position] == captureDevicePosition) break;
    }
	if (!captureDevice) {
        NSLog(@"Unable to acquire video capture device (with position %s).\n", (captureDevicePosition == AVCaptureDevicePositionBack ? "rear" : "front"));
        goto bail0;
    }
